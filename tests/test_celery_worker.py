...
def test_celery_app_initialization() -> None:
    ...
    assert hasattr(celery_worker, "celery_app"), "O módulo celery_worker deve expor 'celery_app'."
...
def test_celery_worker_complete_execution_and_tasks() -> None:
    ...
    with patch("disparo_automatico.enviar_relatorio_direto", return_value=True, create=True), \
            ...
            for task_name, task_obj in celery_worker.celery_app.tasks.items():
                if not task_name.startswith("celery."):
                    try:
                        ...
                        logger.warning(f"Task '{task_name}' executou com exceção controlada via mock: {exc}")
        
        assert tasks_executadas >= 0, "O worker deve conter tasks válidas para varredura."
...
def test_celery_tasks_execution_direct() -> None:
    ...
    with patch("automacao.executar_pipeline", return_value=True, create=True), \
         patch("disparo_automatico.enviar_relatorio_direto", return_value=True, create=True):
        ...
        try:
            res2 = celery_worker.disparar_relatorio_async("teste@empresa.com", {"k": "v"})
