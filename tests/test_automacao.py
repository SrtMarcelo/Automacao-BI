...
def test_enviar_relatorio_direto_sucesso():
    ...
    with patch("pandas.DataFrame.to_excel"), patch(
        "smtplib.SMTP_SSL"
    ) as mock_smtp, patch("os.path.exists", return_value=True), patch(
        "os.remove"
    ), patch(
        "builtins.open", mock_open(read_data=b"conteudo_fake_excel")
    ):
...
def test_enviar_relatorio_direto_falha_conexao():
    ...
    with patch("pandas.DataFrame.to_excel"), patch(
        "smtplib.SMTP_SSL", side_effect=Exception("Erro de conexão SMTP")
