...
def test_enviar_relatorio_fluxo_producao_real(mock_get, mock_smtp) -> None:
    ...
    with patch.dict(
        os.environ, {"SAP_API_URL": "https://api-sap-producao.empresa.com/odata"}
    ):
...
def test_enviar_pesagem_sap_async_mock(mock_post) -> None:
    ...
    with patch.dict(os.environ, {"SAP_API_URL": "https://seu-ambiente-sap/mock"}):
        resultado = disparo_automatico.enviar_pesagem_sap_async.run(
            {"chave_acesso": "352609..."}
        )
...
def test_enviar_pesagem_sap_async_retry_servidor(mock_post) -> None:
    ...
    with patch.dict(
        ...
        with patch.object(
            disparo_automatico.enviar_pesagem_sap_async,
            "retry",
            side_effect=Exception("Retry acionado com sucesso"),
        ):
            with pytest.raises(Exception, match="Retry acionado com sucesso"):
                disparo_automatico.enviar_pesagem_sap_async.run(
                    {"chave_acesso": "12345"}
                )
...
def test_bloco_finally_limpeza_com_excecao_os() -> None:
    ...
    with (
        ...
        if os.path.exists(nome_arquivo):
            ...
            def test_app_main_execution() -> None:
                """Testa a chamada do bloco principal do app.py para cobrir 100% do arquivo."""
    ...
    with patch("app.app.run") as mock_run:
        ...
        def test_app_main_execution() -> None:
            """Testa a execução do bloco principal do app.py para cobrir as linhas restantes."""
