...
def validar_nota_fiscal_produtor(
    chave_acesso: str, peso_bruto: float, tara: float
) -> float:
    ...
    if not chave_acesso or not re.match(r"^\d{44}$", chave_acesso):
        raise RegraFiscalError(
            "Chave de acesso da Nota Fiscal inválida: deve conter exatamente 44 dígitos."
        )

    if peso_bruto <= tara:
        raise RegraFiscalError(
            "Peso bruto não pode ser menor ou igual à tara do veículo."
        )
