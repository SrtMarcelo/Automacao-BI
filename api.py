...
def consultar_dados_por_tenant(tenant_id: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, id_balanca, peso FROM pesagens WHERE tenant_id = ?", (tenant_id,)
    )
...
def listar_pesagens(
    ...
    if not x_tenant_id:
        raise HTTPException(
            status_code=400, detail="Tenant ID obrigatório no cabeçalho."
        )

    dados = consultar_dados_por_tenant(x_tenant_id)

    return {"tenant": x_tenant_id, "total_registros": len(dados), "dados": dados}
